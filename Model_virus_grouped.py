...
logging.info(f'\t dataset studies = {dataset_file["Study_ID"].unique()}')
print(dataset_file['Study_ID'].nunique())

dataset_features = dataset_file.drop(columns=['Participant ID', 'Label_HAI', 'Study_ID'])
X = dataset_features.to_numpy()
logging.info(f'\t dataset features = {dataset_features.columns.tolist()}')
...
skf = StratifiedKFold(n_splits=k_hout, shuffle=True, random_state=random_state_inner_cv)
